...
<body>
	...
	<div class="container-fluid admin">
		<div class="col-md-12 alert alert-primary">Minha Lista de Questionários</div>
		<br>
		<div class="card">
			<div class="card-body">
				<table class="table table-bordered" id='table'>
					...
					<tr>
						...
						<td><?php echo $status->num_rows > 0 ? 'Feito' : 'Pendente' ?></td>
						<td>
							<center>
							 	<?php if($status->num_rows <= 0): ?>
							 	<a class="btn btn-sm btn-outline-primary" href="./answer_sheet.php?id=<?php echo $row['id']?>"><i class="fa fa-pencil"></i> Realizar Teste</a>
								<?php else: ?>
								<a class="btn btn-sm btn-outline-primary" href="./view_answer.php?id=<?php echo $row['id']?>"><i class="fa fa-eye"></i> Ver</a>
							<?php endif; ?>
							</center>
						</td>
					</tr>
...
<script>
	$(document).ready(function(){
		...
		$('.remove_faculty').click(function(){
			var id = $(this).attr('data-id')
			var conf = confirm('Tem certeza que deseja deletar esse dado?');
			if(conf == true){
				$.ajax({
